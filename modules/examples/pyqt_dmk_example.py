...
def LeftCallback(hGrabber, pBuffer, framenumber, pData):
    ...
    if pData.getNextImage == 1:
        ...
        if pData.buffer_size > 0:
            ...
            third = int(len(image.contents)/(pData.height.value*pData.width.value))
            print(third, pData.height.value, pData.width.value)
...
def SelectLeftDevice():
    ...
    if ic.IC_IsDevValid(LeftCamera): 
        ic.IC_SetHWnd(LeftCamera,  int(LeftVideo.winId()))
...
#         ic.IC_SetHWnd(RightCamera, int(RightVideo.winId()))
...
def OnSnap():
    ...
    while LeftUserData.getNextImage != 0:
        # print('0')
        time.sleep(0.001)
    ...
    cv2.imwrite("Left.tif", cv2.flip(LeftUserData.cvMat, 0))
    cv2.imwrite("Left2.tif", cv2.flip(LeftUserData.cvMat>>4, 0))
...
SnapButton = qt5.QPushButton("Snap Image")
SnapButton.clicked.connect(OnSnap)
...
w.show()
print(LeftVideo.winId(), type(LeftVideo.winId()))
